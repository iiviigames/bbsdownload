// ==UserScript==
// @name         Picotron/Pico-8 Cart Download Button
// @namespace    https://github.com/iiviigames/bbsdownload
// @version      0.7
// @match        *://*.lexaloffle.com/id/*
// @match        *://www.lexaloffle.com/bbs/?cat=7/*
// @match        *://www.lexaloffle.com/bbs/?cat=8/*
// @match        *://www.lexaloffle.com/bbs/*
// @author       iivii | iiviigames@pm.me | @iiviigames
// @website      https://twitch.tv/iiviigames
// @homepageURL  https://github.com/iiviigames/bbsdownload
// @updateURL    https://github.com/iiviigames/bbsdownload/raw/master/Pico-8%20Download%20Cart.user.js
...
function dbgmain() {
  ...
  var link_front = "https://www.lexaloffle.com/bbs/cposts/";
  var link_end = ".p8.png";
  var link_end_new = findCartStorageChars();
...
// Determines if this is a picotron or p8 cart
function findCartStorageChars(){
  ...
  // Get the third one.
  var ext = splits[1];
  //if (ext != 'p8' || ext != 'p64') {ext = splits[0]};
...
function cartDownload(cart_name, cart_download_link){
  ...
  var link_end = ".png";
  var chk = cart_download_link
  var is8 = chk[chk.search(/p8|p64/)+1];
  //a[a.search(/p8|p64/)+1]
  link_end = is8 == "8" ? '.p8'+link_end : '.p64'+link_end;
  ...
  // This is why we needed the cart's name separately.
  // The BBS dynamically names elements within every cart page with
  // PHP, and thus they are never the same. This made the work
  // much harder as it uses the unique ID to name MANY, MANY parts of 
  // the css and html elements needed to make any of this function.
...
// TODO: IMPLEMENT SOURCE CODE VIEWING
